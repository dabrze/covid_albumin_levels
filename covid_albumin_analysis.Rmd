...
get_last_samples <- function(df_all_samples) {
  df <- df_all_samples %>%
    group_by(PATIENT_ID) %>% 
    summarise(albumin=last(albumin), OutcomeFactor=last(OutcomeFactor), OutcomeNumber=last(outcome),
...
get_first_samples <- function(df_all_samples) {
  df <- df_all_samples %>%
    group_by(PATIENT_ID) %>% 
    summarise(albumin=first(albumin), OutcomeFactor=first(OutcomeFactor), OutcomeNumber=first(outcome),
...
draw_albumin_glucose_plot <-function(df) {
  p <- ggplot(df, aes(x=glucose, y=albumin, color=OutcomeFactor)) + 
  geom_hline(aes(yintercept=35), colour=NORMAL_RANGE_COLOR, linetype="dashed") +
  geom_hline(aes(yintercept=55), colour=NORMAL_RANGE_COLOR, linetype="dashed") +
...
save_plot(albumin_glucose_plot + theme(legend.position="none"), "albumin_glucose_plot")
```
...
```{r}
# TODO: https://rpubs.com/amertens/a
logistic.fit.albumin<-glm(OutcomeNumber~albumin, family="binomial", data=df_last)
summary(logistic.fit.albumin)
exp(logistic.fit.albumin$coefficients)
```
```{r}

logistic.fit.glucose<-glm(OutcomeNumber~glucose, family="binomial", data=df_last)
summary(logistic.fit.glucose)
exp(logistic.fit.glucose$coefficients)
```
```{r}
logistic.fit.age<-glm(OutcomeNumber~age, family="binomial", data=df_last)
summary(logistic.fit.age)
exp(logistic.fit.age$coefficients)
```

```{r}
logistic.fit.gender<-glm(OutcomeNumber~gender, family="binomial", data=df_last)
summary(logistic.fit.gender)
exp(logistic.fit.gender$coefficients)
```
```{r}
logistic.fit.adjusted<-glm(OutcomeNumber~albumin + glucose + age + gender, family="binomial", data=df_last)
summary(logistic.fit.adjusted)
exp(logistic.fit.adjusted$coefficients)
```

```{r}
logistic.fit.effect<-glm(OutcomeNumber~albumin * gender, family="binomial", data=df_last)
summary(logistic.fit.effect)
exp(logistic.fit.effect$coefficients)
```
...
gender_albumin_regression <- ggplot(df_last, aes(x=albumin, y=OutcomeNumber, color=gender)) + 
  ...
  xlab ("Patient's albumin level (g/L)") +
  labs(color = "Gender") + 
