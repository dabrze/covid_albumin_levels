...
read_data <- function(file) {
  ...
  df_all_samples <- df %>%
    mutate(gender = replace(gender, gender == 1, "Male"), 
           ...
           OutcomeFactor = factor(OutcomeFactor, levels = c("Died", "Survived")),
...
mortality_rate <- nrow(df_last[df_last$OutcomeFactor == "Died",])/nrow(df_last) 
mortality_rate_male <- nrow(df_last[df_last$OutcomeFactor == "Died" & df_last$gender == "Male",])/nrow(df_last[df_last$gender == "Male",]) 
mortality_rate_female <- nrow(df_last[df_last$OutcomeFactor == "Died" & df_last$gender == "Female",])/nrow(df_last[df_last$gender == "Female",]) 
```

The data for the study was taken from the supplementary materials of "An interpretable mortality prediction model for COVID-19 patients. Nat. Mach. Intell. 2, 283–288 (2020)" by *Yan et al.* and describes COVID-19 patients admitted to Tongji Hospital, Wuhan, China between January 10 and February 18, 2020. The raw dataset contained information about `r raw_patients` patients, however only `r albumin_patients` patients that had their albumin levels measured at least once during their hospital stay, and those were of interest to the current study. The table below presents the basic statistics of clinical variables analyzed in this study.

```{r stats, results='asis'}
Tongji <- df_last %>% select(OutcomeFactor, gender, age, TimeSinceSubmission, albumin, glucose) %>%
  rename(Outcome = OutcomeFactor, Gender = gender, Age = age, TimeSinceSubmission=TimeSinceSubmission, Albumin = albumin, Glucose = glucose)
